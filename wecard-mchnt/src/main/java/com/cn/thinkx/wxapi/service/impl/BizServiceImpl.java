...
import com.cn.thinkx.core.util.Constants.FansStatusEnum;
...
//		}
	...
	private String processEventMsg(MsgRequest msgRequest, MpAccount mpAccount, boolean merge) {
		...
		if (MsgType.SUBSCRIBE.toString().equals(msgRequest.getEvent())) {// 订阅消息
			...
			if (text != null) {
				AccountFans accountFans = accountFansService.getByOpenId( msgRequest.getFromUserName());
//				if(accountFans == null || FansStatusEnum.Fans_STATUS_00.getCode().equals(accountFans.getFansStatus())){ //判断是否已经有菜单权限
//					text.setContent(WxUtil.contentAddhref(text.getContent(), WxUtil.getProjectUrl()+"/wxclient/mchnt/mchtRegister.html", Contents.WX_MERCHANT_REG_NAME, msgRequest.getFromUserName()));
//				}
				/***移动用户分组**/
				if(accountFans !=null && FansStatusEnum.Fans_STATUS_10.getCode().equals(accountFans.getFansStatus()) && !"0".equals(accountFans.getGroupid())){
					this.updateFansGroupId(msgRequest.getFromUserName(),  "100", mpAccount);
				}
	...
	public AccountFans syncAccountFans(String openId, MpAccount mpAccount, boolean merge) {
		...
		if (merge && null != fans) {
			...
			if (tmpFans == null) {
				...
				fans.setFansStatus(Constants.FansStatusEnum.Fans_STATUS_00.getCode());
				...
				//同步商户
				try{
					if(fans.getGroupid() !=null && ! Constants.GroupsIdStatEnum.groupdefauls_stat.getCode().equals(fans.getGroupid())){
