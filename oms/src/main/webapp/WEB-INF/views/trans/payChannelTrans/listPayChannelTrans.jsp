...
<html>
	<head>
		...
		<script src="${ctx}/resource/js/module/trans/payChannelTrans/listPayChannelTrans.js"></script>
	</head>
	<body>
		...
		<div id="contentwrapper">
			<div class="main_content">
				...
				<form id="searchForm" action="${ctx }/trans/payChannelTransInf/listTrans.do" class="form-inline" method="post">
					<h3 class="heading">通道交易明细列表</h3>
					<div class="row-fluid">
						<div class="span12">
...
<%-- 									<td>${entity.shopName}</td> --%>
									...
									<sec:authorize access="hasRole('ROLE_SCAN_BOX_DEVICE_INF_DELETE')">
										<c:if test="${entity.respCode == '交易成功' && entity.transId =='聚合支付消费' && entity.transSt != 3}">
										<a dmsRelatedKey="${entity.wxPrimaryKey}"  title="退款" class="btn-mini btn-edit" href="#"><i class="icon-edit"></i></a>
										</c:if>
										<c:if test="${entity.respCode == '交易成功' && entity.transId =='聚合支付消费' && entity.transSt == 3}">
										已退款
										</c:if>
