...
<head>
    ...
    <script src="${ctx}/resource/js/module/statement/merchantStatement/listShopDetail.js"></script>
</head>
<body>
	   <%@ include file="/WEB-INF/views/common/navbar.jsp"%>
            <div id="contentwrapper">
                <div class="main_content">
					...
					<form id="searchForm" action="${ctx }/statement/merchantStatement/listShopDetail.do" class="form-inline form_validation_tip" method="post">
						<input type="hidden" id="operStatus"  value="${operStatus }"/>
						<h3 class="heading">档口交易流水列表</h3>
						...
						<div class="row-fluid" >
							 <div class="span12">
                                ...
                                <div class="input-prepend">
                                    <span class="add-on">选择档口:</span>
                                    <select name="positCode" id="positCode" class="input-medium">
                                    <c:if test="${condition.shopCode!=null }">
                                         <c:forEach var="posit" items="${positList}" varStatus="st">
                                                <option value="${posit.shopCode}"  <c:if test="${posit.shopCode==condition.positCode}">selected</c:if> >${posit.shopName} </option>
                                         </c:forEach>
                                    </c:if>
                                    </select>
                                </div>
