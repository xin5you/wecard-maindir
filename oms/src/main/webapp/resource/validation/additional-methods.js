...
//匹配密码，长度在6-20之间，只能包含字符、数字和下划线。
jQuery.validator.addMethod("isPwd", function(value, element) {
	var pwd = /^[a-zA-Z0-9_]{6,20}$/;
	return this.optional(element) || (pwd.test(value));
}, "密码只能使用字母、数字和下划线，长度在6-20个字符");

//安全码必须为6位数字
jQuery.validator.addMethod("sixSafeCode", function(value, element) {
	var pwd = /^\d{6}$/;
	return this.optional(element) || (pwd.test(value));
}, "安全码必须为6位数字");
...
}, "用户名只能使用字母、数字和下划线，长度在6-16个字符，请检查您输入的用户名！");

jQuery.validator.addMethod("isSafeKey", function(value, element) {
	var reg = /^[a-zA-Z0-9]{6,32}$/;
	return this.optional(element) || (reg.test(value));
}, "Key只能使用字母或数字，长度在6-32个字符");
...
//提现金额
jQuery.validator.addMethod("isPayAmount", function(value, element) {
	var length = value.length;
	return this.optional(element) || (/^\d{1,10}(\.\d{1,2})?$/.test(value));
}, "请输入正确的提现金额");

//提现金额
jQuery.validator.addMethod("isPayRate", function(value, element) {
	var length = value.length;
...
jQuery.validator.addMethod("notLessTo", function(value, element, param) {
  var target = $(param);
  var targetVal = parseFloat(target.val());
  return this.optional(element) || (value >= targetVal);   
}, $.validator.format("开始值不能小于结束值"));
