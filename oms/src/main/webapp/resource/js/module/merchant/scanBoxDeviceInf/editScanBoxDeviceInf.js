$(document).ready(function () {
	editScanBoxDeviceInf.init();
})

var editScanBoxDeviceInf = {
	init:function(){
		editScanBoxDeviceInf.initTip();
		editScanBoxDeviceInf.initEvent();
	},
	initEvent: function () {
			$('#mercahnt_select').change(editScanBoxDeviceInf.getShopInfList);
			$('#shopCode1').change(editScanBoxDeviceInf.getShopInfList2);
	},
    initTip: function () {
        var ttip_validator = $('.form_validation_tip').validate({
            ...
            submitHandler: function(form) {
            	editScanBoxDeviceInf.editScanBoxDeviceInfCommit();
            },
	...
	editScanBoxDeviceInfCommit:function(){
	            ...
	            success: function(data){
	            	if(data.status){
	            		$("#mainForm").ajaxSubmit({
	        	            ...
	        	            url: Helper.getRootPath() + '/merchant/scanBoxDeviceInf/editScanBoxDeviceInfCommit.do', // 需要提交的 url
	        	            dataType: 'json',
	        	            success: function(data){
	        	                if(data.status) {
	        	                	location.href=Helper.getRootPath() + '/merchant/scanBoxDeviceInf/listScanBoxDeviceInf.do?operStatus=2';
	        	                }else{
	        	                	Helper.alert(data.msg);
	        	                	return false;
	        	                }
