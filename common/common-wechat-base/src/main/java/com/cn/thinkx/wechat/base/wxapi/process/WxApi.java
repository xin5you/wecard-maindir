...
public class WxApi {
	...
	private static final String GET_VALID_ACCESS_TOKEN = "https://api.weixin.qq.com/sns/auth?access_token=%s&openid=%s";
	...
	// 创建分组
	// private static final String CREATE_GROUPS = "https://api.weixin.qq.com/cgi-bin/groups/create?access_token=%s";
	
	// 查询所有分组
	// private static final String GET_GROUPS = "https://api.weixin.qq.com/cgi-bin/groups/get?access_token=%s";
	
	// 移动用户所在分组
	private static final String UPDATE_MEMBERS_GORUPS = "https://api.weixin.qq.com/cgi-bin/groups/members/update?access_token=%s";
	
	// 长链接转短链接接口
	private static final String LONG_TO_SHORT = "https://api.weixin.qq.com/cgi-bin/shorturl?access_token=%s";
	
	//授权码查询openid
	private static final String AUTHCODE_TO_OPENID="https://api.mch.weixin.qq.com/tools/authcodetoopenid";
	 ...
	 * @return
	 */
	public static String getValidAccessTokenUrl(String token, String openId) {
		return String.format(GET_VALID_ACCESS_TOKEN, token, openId);
	}
	...
	// 移动用户分组 url
	public static String getUpdateMembersGroupsUrl(String token){
		return String.format(UPDATE_MEMBERS_GORUPS, token);
	}
	
	// 获取长链接转短链接url
	public static String getLongToShort(String token){
		return String.format(LONG_TO_SHORT, token);
	}
	
	// 授权码查询openid
	public static String getAuthCodeToOpenId(){
		return AUTHCODE_TO_OPENID;
	}
	...
	// 获取短链接
	public static String getShortUrlJson(String longUrl) {
		return "{\"action\":\"long2short\",\"long_url\":\""+ longUrl + "\"}";
	}
	...
	/**
	 * 获取校验access_token返回信息
	 * @param token
	 * @param openId
	 * @return
	 */
	public static AccessToken getValidAccessToken(String token, String openId) {
		AccessToken accessToken = null;
		String tockenUrl = getValidAccessTokenUrl(token, openId);
		JSONObject jsonObject = httpsRequest(tockenUrl, HttpMethod.GET, null);
		if (null != jsonObject && !jsonObject.containsKey("errcode")) {
			try {
				...
				accessToken = null;// 获取校验access_token返回信息失败
			}
		} else if (null != jsonObject) {
			accessToken = new AccessToken();
			accessToken.setErrcode(jsonObject.getInt("errcode"));
	...
	public static JSONObject httpsRequest(String requestUrl, String requestMethod) {
		return JSONObject.fromObject(httpsRequest(requestUrl, requestMethod, null));
	}
	
	public static JSONObject httpsRequest(String requestUrl, String requestMethod, String outputStr) {
		return JSONObject.fromObject(httpsRequestBase(requestUrl, requestMethod, outputStr));
	}

	public static String httpsRequestBase(String requestUrl, String requestMethod, String outputStr) {
		StringBuffer buffer = new StringBuffer();
		try {
			TrustManager[] tm = { new WecardX509TrustManager() };
			...
			if (conn == null)
				throw new Exception("===地址[" + url + "]网络连接异常===");
...
class WecardX509TrustManager implements X509TrustManager {
