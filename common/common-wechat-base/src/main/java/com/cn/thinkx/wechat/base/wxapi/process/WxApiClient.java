...
public class WxApiClient {
	 ...
	 * @return
	 */
	public static List<MpAccount> getAllMpAccountList() {
		return WxMemoryCacheClient.getAllMpAccountMaps();
	}

	// 获取accessToken
	public static String getAccessToken(MpAccount mpAccount) {
		// 获取唯一的accessToken，如果是多账号，请自行处理
		AccessToken token = WxMemoryCacheClient.getSingleAccessToken(mpAccount.getAccount());
		if (token != null && !token.isExpires()) {// 不为空，并且没有过期
			...
			if (token != null) {
				if (token.getErrcode() != null) {// 获取失败
					logger.error("## getAccessToken Errcode[{}] ErrorMsg [{}]", token.getErrcode(), token.getErrmsg());
				} else {
	...
	public static MpAccount getMpAccount(String account) {
		return WxMemoryCacheClient.getMpAccount(account);
	}

	// 刷新AccessToken
	public static void doRefreshAccessToken(MpAccount mpAccount) {
		// 获取唯一的accessToken，如果是多账号，请自行处理
		AccessToken token = WxApi.getAccessToken(mpAccount.getAppid(), mpAccount.getAppsecret());
		if (token != null) {
			if (token.getErrcode() != null) {// 获取失败
				logger.error("## getAccessToken Error [{}]", JSONObject.fromObject(token).toString());
			} else {
				WxMemoryCacheClient.addAccessToken(mpAccount.getAccount(), token);
			}
		}
	...
	// 刷新jsToken
	public static void doRefreshJsToken(MpAccount mpAccount) {
		String token = getAccessToken(mpAccount);
		JSTicket jsTicket = WxApi.getJSTicket(token);
		if (jsTicket != null) {
	...
	public static String getJSTicket(MpAccount mpAccount) {
		...
		JSTicket jsTicket = WxMemoryCacheClient.getSingleJSTicket(mpAccount.getAccount());
	 ...
	 * @return
	 */
	public static JSONObject sendCustomerNewsMessage(String openid, List<MsgNews> msgNews, MpAccount mpAccount) {
		if (!StringUtils.isBlank(openid) && msgNews != null && msgNews.size() > 0) {
			String accessToken = getAccessToken(mpAccount);
			String content = WxMessageBuilder.prepareCustomerNews(openid, msgNews);
			return WxApi.httpsRequest(WxApi.getSendCustomMessageUrl(accessToken), HttpMethod.POST, content);
		}
		return null;
	}
	...
	public static JSONObject sendTemplateMessage(TemplateMessage tplMsg, MpAccount mpAccount) {
		if (tplMsg != null) {
			String accessToken = getAccessToken(mpAccount);
			return WxApi.httpsRequest(WxApi.getSendTemplateMessageUrl(accessToken), HttpMethod.POST, 
					JSON.toJSONString(tplMsg));
		}
		return null;
	}
