            ...
            if (!tmpElement.isTextOnly()) {
                ...
                for (Iterator it = set.iterator(); it.hasNext(); ) {
                    ...
                    if (rsltMapp.containsKey(key)) {
                        ...
                        rsltMapp.get(key);
                        if(rsltMapp.get(key) instanceof ArrayList) {
                            for (Object map : (List<Object>) rsltMapp.get(key)) {
                                rsltList.add(map);
                            }
                        }else{
                            rsltList.add(rsltMapp.get(key));
                        }
                        rsltList.add( rsltMap.get(key));
