package com.cn.thinkx.wecard.customer.core.spring;
...
/**
 * request.getContextPath() 使用nginx做反向代理的时候(比如 设置访问链接、图片链接的时候)，不需要contextPath的；
 * 
 * 因为此项目使用freemaker，对应的文件是spring.ftl文件； 所以以此来判断是否用户自己设置了 contextPath：
 * 如果设置了，返回null；业务直接使用 request.getContextPath() 如果设置为空，返回 ""; 业务直接使用""
 */
