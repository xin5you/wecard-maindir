    ...
    webSocket.onmessage = function(event){
    	var data=JSON.parse(event.data);
    	 ...
    	 if(data.sendType=='20' && data.payType=='VIPCARD_PAY'){  //请输入密码的请求 会员卡支付
    		 	//分转元
    		 	var yuan = Math.round(data.transAmt);
    			if (!yuan) {
    				return "";
    			}
    			yuan = yuan.toString()
...
//    						    	$.showLoading('正在请求支付.....');
    					        	    ...
    					        	    success:function(data2){
    					        	    	if(data2 != null && data2 != '') {
    					        	    		if (data2.code == "00") {// 交易成功
    					        	    			location.href = CONETXT_PATH + '/pub/paySuccess.html?transId=W71&transAmt='+data2.transAmt+"&wxTransLogKey="+wxTransLog.wxTransLogKey;
    					        	    		} else {
    		...
    		if(data.sendType=='90'){  //支付结果
    			if(data.code=='00'){
    				 window.location.href = CONETXT_PATH + '/pub/paySuccess.html?transId=W10&transAmt='+data.transAmt+"&wxTransLogKey="+data.wxTransLogKey;
    			}else{
    				 window.location.href = CONETXT_PATH + '/pub/payFailed.html?transId=W10&transAmt='+data.transAmt+'&transCode='+data.code+'&mchntCode='+data.mchntCode;
    			}
    		}
    };
	...
	var pageManager = {
        ...
        genQrcode: function() {
//        	var payType='VIPCARD_PAY';
        	var payType=$('#payType').val();
        	$.ajax({
        		url : CONETXT_PATH + '/customer/user/genCustomerQrcode.html',
        		type : 'post',
    	        dataType : "json",
    	        data:{
	                  'payType':payType
